...
@implementation UIImage (Phippy)

- (UIImage *)compressWithTargetSize:(CGSize)size{
    ...
    if( ( imageHeight - imageWidth)>0) {
        ...
        CGFloat tempW = size.width;
        
        CGFloat tempH = size.height;
    ...
    CGFloat targetWidth = size.width;//获取最终的目标宽度尺寸
    
    CGFloat targetHeight = size.height;//获取最终的目标高度尺寸
    ...
    UIGraphicsBeginImageContext(size);//开始剪切
